...
class Serial_connection(serial.Serial):
...
class Pump2000:  # defines Pump2000 class with serial connection, address, and name (used in logging)
    ...
    def write_read(self, command, bytes=20):  # writes commands to pump, and gets response
    ...
    def version(self, bytes=60):  # tests serial connection to pump at a certain address
        ...
        if (self.address[0] == '0'):  # address is only 1 digit
            if (version_pump[-2:-1] == self.address[1]):  # last character on response matches address
    ...
    def set_dia(self, diameter, bytes=60):  # sets diameter
        ...
        if (float(diam_response) == float(remove_string_extras(str(diameter)))):  # checks if currently set diameter
    ...
    def set_infuse_rate(self, infuse_rate, infuse_rate_units):  # sets the infuse rate (flow rate out from syringe)
        ...
        if (float(remove_string_extras(irate_resp[3:9]).split(' ')[0]) == float(
                infuse_rate)):  # checks updated flow rate matches newly set flow rate
            ...
            logging.error('%s: set infuse rate (%s %s) does not match flowrate returned by pump (%s)',
                          self.name, str(infuse_rate), infuse_rate_units,
    ...
    def set_withdraw_rate(self, withdraw_rate, withdraw_rate_units):  # sets the withdraw rate (flow rate into syringe)
        ...
        if "Out of range" in wrate:  # if flow rate is out of the range for the syringe's diameter
            logging.error('%s: withdraw rate (%s %s) is out of range', self.name, str(withdraw_rate),
                          withdraw_rate_units)
            self.serialcon.close()
            raise pumperror("withdraw rate is out of range")
        if (float(remove_string_extras(wrate_resp[3:9]).split(' ')[0]) == float(
                withdraw_rate)):  # checks updated flow rate matches newly set flow rate
            ...
            logging.error('%s: set withdraw rate (%s %s) does not match flowrate returned by pump (%s)',
                          self.name, str(withdraw_rate), withdraw_rate_units,
    ...
    def set_target_volume(self, target_volume, target_volume_units, bytes=60):  # sets the target volume for a pump
        ...
        if (target_volume_units == 'ul'):  # need to convert target volume (from ul to ml) if target volume unit is ul
        ...
        logging.info('%s: target volume set to %s %s', self.name, str(target_volume), target_volume_units)

    def set_pump_mode(self, bytes=60):  # if no target volume is set, then pump needs to be set to pump mode
    ...
    def set_syringe_volume(self, syringe_volume, syringe_volume_units, bytes=60):  # sets the syringe volume for a pump
        if (
                syringe_volume_units == 'ul'):  # need to convert syringe volume (from ul to ml) if syringe volume
            # unit is ul
        ...
        logging.info('%s: syringe volume set to %s %s', self.name, str(syringe_volume), syringe_volume_units)

    def set_stop(self, bytes=60):  # stops pump
        stop_pump = self.write_read('STP', bytes=bytes)
        if (stop_pump[-1] == ":" or stop_pump[-1] == "*"):  # checks if pump has correctly stopped
    ...
    def set_poll(self, i_or_w, bytes=60):  # checks volume delivered
        ...
        if (poll_pump[-1] == ":" or poll_pump[-1] == "*"):  # pump has stopped
            ...
            logging.info('%s: has not reached target volume and is still infusing', self.name)
...
class PumpUltra:  # defines PumpUltra class with serial connection, address, and name (used in logging)
    ...
    def version(self, bytes=60):  # tests serial connection to pump at a certain address
        ...
        if (version_pump[-1] == "*"):  # target volume had been reached, need to clear target volume then try again
            self.clear_target_volume(bytes=60)
            version_pump = self.write_read('ver', bytes=bytes)
        if (version_pump[-3:-1] == self.address):  # last 2 characters on response matches address
            logging.info('%s: created at address %s', self.name, self.address)
        else:
            logging.error('%s: not connected to pump correctly at address %s', self.name, self.address)
            self.serialcon.close()
    ...
    def set_dia(self, diameter, bytes=60):  # sets diameter
        ...
        if (float(dia_response) == float(remove_string_extras(
                str(diameter)))):  # checks if currently set diameter matches the new diameter argument
            logging.info(
                '%s: did not update diameter because set diameter (%s mm) already matches diameter returned by pump ('
                '%s mm)',
                self.name, str(diameter), dia_response)
    ...
    def set_withdraw_rate(self, withdraw_rate, withdraw_rate_units):  # sets the withdraw rate (flow rate into syringe)
        if (len(remove_string_extras(str(withdraw_rate))) > 6):  # checks if flow rate needs to be truncated
            ...
            logging.info('%s: set withdraw rate (%s %s) has been truncated to %s %s',
                         self.name, str(withdraw_rate), withdraw_rate_units, str(new_wr), withdraw_rate_units)
            withdraw_rate = new_wr
        wrate = self.write_read('wrate ' + str(withdraw_rate) + ' ' + withdraw_rate_units,
                                bytes=60)  # set withdraw rate
        ...
        if (float(remove_string_extras(wrate_respr[0:12]).split(' ')[0]) == float(
                str(withdraw_rate))):  # checks updated flow rate matches newly set flow rate
            logging.info('%s: withdraw rate set to %s %s', self.name, str(withdraw_rate), withdraw_rate_units)
        else:
            logging.error('%s: set withdraw rate (%s %s) does not match flowrate returned by pump (%s)',
                          self.name, str(withdraw_rate), withdraw_rate_units,
    ...
    def set_syringe_volume(self, syringe_volume, syringe_volume_units, bytes=60):  # sets the syringe volume for a pump
        if (len(remove_string_extras(str(syringe_volume))) > 6):  # checks if syringe volume needs to be truncated
            ...
            syringe_volume = new_tv
        self.write_read('svolume ' + str(syringe_volume) + ' ' + syringe_volume_units,
                        bytes=bytes)  # sets syringe volume
        logging.info('%s: syringe volume set to %s %s', self.name, str(syringe_volume), syringe_volume_units)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='pump_code_pack controls syringe pumps from the command line.')
    ...
    group.add_argument('-withdraw_wait', action="store_true",
                       help='set pump to infuse and wait until target volume has been reached')
    group.add_argument('-poll', action="store_true", help='check if pump has reached target volume')
    ...
    chain = Serial_connection(args.usbport)

    address = '{0:02.0f}'.format(int(args.address))
    try:
        if args.PHDULTRA:  # if PHDULTRA is an argument from the command line
            pump_ultra = PumpUltra(chain, args.address, name='PHDULTRA')  # define PumpUltra class
            if args.stop:  # check if stop is an argument
                ...
                if args.withdraw:
                    wrun = pump_ultra.set_wrun(bytes=60)  # runs the pump in the withdraw direction
                if args.infuse_wait:
                    iw = pump_ultra.wait_for_target(i_or_w="infuse",
                                                    bytes=60)  # runs pump in the infuse direction then waits for
                    # target
                if args.withdraw_wait:
                    ww = pump_ultra.wait_for_target(i_or_w="withdraw", bytes=60)  # runs pump in the infuse direction
        ...
        if args.PHD2000:  # if PHDULTRA is an argument from the command line
            ...
            pump_2000 = Pump2000(chain, args.address, name='PHD2000')  # define Pump2000 class
            if args.stop:  # check if stop is an argument
                stop_pump = pump_2000.set_stop(bytes=30)
            elif args.poll:  # check if poll is an argument
                poll_pump = pump_2000.set_poll(i_or_w="infuse", bytes=30)
            elif args.poll_infuse:  # check if poll is an argument (exact same at normal poll for phd2000)
                poll_pump = pump_2000.set_poll(i_or_w="infuse", bytes=30)
            elif args.poll_withdraw:  # check if poll is an argument (exact same at normal poll for phd2000)
                poll_pump = pump_2000.set_poll(i_or_w="infuse", bytes=30)
            else:
                if args.diameter:  # if diameter is an argument, set diameter at pump
                    dia_pump = pump_2000.set_dia(args.diameter, bytes=30)
                if args.syringe_volume:  # if syringe volume is an arugment, set syringe volume
                    sv_pump = pump_2000.set_syringe_volume(args.syringe_volume, args.syringe_volume_units, bytes=30)
                if args.target_volume:  # if target volume is an arugment, set target volume
                    tv_pump = pump_2000.set_target_volume(args.target_volume, args.target_volume_units, bytes=30)
                ...
                if args.withdraw:  # if withdraw is an argument then set the withdraw rate, and withdraw
                    if args.withdraw_rate:  # if withdraw rate is an arugment, set infuse rate
                        if args.withdraw_rate_units:  # also needs withdraw rate units
                ...
                if args.infuse_wait:  # infuse then wait for target volume
                    if args.target_volume:
                        iw = pump_2000.wait_for_target(i_or_w="infuse",
                                                       bytes=30)  # runs the pump then waits for target
                    else:
                    ...
                    if args.target_volume:
                        if args.withdraw_rate:
                            if args.withdraw_rate_units:
                                ...
                                chain.close()
                                raise pumperror("Need withdraw rate units")
                        ww = pump_2000.wait_for_target(i_or_w="withdraw",
                                                       bytes=30)  # runs the pump then waits for target
                    else:
                        logging.error('%s: Need target volume', pump_2000.name)
                        chain.close()
